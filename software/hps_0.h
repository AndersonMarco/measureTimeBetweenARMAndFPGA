#ifndef _ALTERA_HPS_0_H_
#define _ALTERA_HPS_0_H_

/*
 * This file was automatically generated by the swinfo2header utility.
 * 
 * Created from SOPC Builder system 'soc_system' in
 * file './soc_system.sopcinfo'.
 */

/*
 * This file contains macros for module 'hps_0' and devices
 * connected to the following masters:
 *   h2f_axi_master
 *   h2f_lw_axi_master
 * 
 * Do not include this header file and another header file created for a
 * different module or master group at the same time.
 * Doing so may result in duplicate macro names.
 * Instead, use the system header file which has macros with unique names.
 */

/*
 * Macros for device 'led_output', class 'altera_avalon_pio'
 * The macros are prefixed with 'LED_OUTPUT_'.
 * The prefix is the slave descriptor.
 */
#define LED_OUTPUT_COMPONENT_TYPE altera_avalon_pio
#define LED_OUTPUT_COMPONENT_NAME led_output
#define LED_OUTPUT_BASE 0x10040
#define LED_OUTPUT_SPAN 16
#define LED_OUTPUT_END 0x1004f
#define LED_OUTPUT_BIT_CLEARING_EDGE_REGISTER 0
#define LED_OUTPUT_BIT_MODIFYING_OUTPUT_REGISTER 0
#define LED_OUTPUT_CAPTURE 0
#define LED_OUTPUT_DATA_WIDTH 32
#define LED_OUTPUT_DO_TEST_BENCH_WIRING 0
#define LED_OUTPUT_DRIVEN_SIM_VALUE 0
#define LED_OUTPUT_EDGE_TYPE NONE
#define LED_OUTPUT_FREQ 50000000
#define LED_OUTPUT_HAS_IN 0
#define LED_OUTPUT_HAS_OUT 1
#define LED_OUTPUT_HAS_TRI 0
#define LED_OUTPUT_IRQ_TYPE NONE
#define LED_OUTPUT_RESET_VALUE 1023

/*
 * Macros for device 'pio_input', class 'altera_avalon_pio'
 * The macros are prefixed with 'PIO_INPUT_'.
 * The prefix is the slave descriptor.
 */
#define PIO_INPUT_COMPONENT_TYPE altera_avalon_pio
#define PIO_INPUT_COMPONENT_NAME pio_input
#define PIO_INPUT_BASE 0x10050
#define PIO_INPUT_SPAN 16
#define PIO_INPUT_END 0x1005f
#define PIO_INPUT_BIT_CLEARING_EDGE_REGISTER 0
#define PIO_INPUT_BIT_MODIFYING_OUTPUT_REGISTER 0
#define PIO_INPUT_CAPTURE 0
#define PIO_INPUT_DATA_WIDTH 32
#define PIO_INPUT_DO_TEST_BENCH_WIRING 0
#define PIO_INPUT_DRIVEN_SIM_VALUE 0
#define PIO_INPUT_EDGE_TYPE NONE
#define PIO_INPUT_FREQ 50000000
#define PIO_INPUT_HAS_IN 1
#define PIO_INPUT_HAS_OUT 0
#define PIO_INPUT_HAS_TRI 0
#define PIO_INPUT_IRQ_TYPE NONE
#define PIO_INPUT_RESET_VALUE 0


#endif /* _ALTERA_HPS_0_H_ */
